...
# # Clear Keras session to prevent memory issues
...
# def load_artifacts():
#     try:
#         # Load the custom Self-Attention model
#         # compile=False allows loading without reconstructing custom training loss configurations
...
#         # Load Label Encoder mapping dictionary
...
#         st.error(
#             f"Error loading model files. Please ensure model.h5, tokenizer.pkl, and label_encoder.pkl are in this folder. Details: {e}"
#         )
#         return None, None, None


# model, tokenizer, category_mapping = load_artifacts()

# # Reverse mapping for display (integer -> category name)
# if category_mapping:
#     reverse_mapping = {v: k.upper() for k, v in category_mapping.items()}


# # ---------------------------------------------------------
# # 2. HELPER PREPROCESSING FUNCTION
# # ---------------------------------------------------------
# def clean_text(text):
#     text = str(text).lower()
...
# # ---------------------------------------------------------
# # 3. STREAMLIT USER INTERACTION UI
# # ---------------------------------------------------------
# st.title("📰 AI-Powered News Classification System")
# st.markdown(
#     """
# This application utilizes a **Multi-Head Self-Attention Neural Network** to read a news article, 
# classify its category, and extract underlying token weights to visually display **word-to-word relationships**.
# """
# )
# st.write("---")

# # Layout: Split into Input area and Results area
# col1, col2 = st.columns([1, 1])

# with col1:
#     st.subheader("Input News Article")
#     user_input = st.text_area(
#         "Paste the full text of the article or type a headline below:",
#         height=250,
#         placeholder="Example: Apple launches new AI-powered iPhone with state-of-the-art features...",
#     )
#     submit_button = st.button("Analyze & Classify Article", type="primary")

# with col2:
#     st.subheader("Classification & Visual Model Explainability")

#     if submit_button and user_input.strip() != "":
#         if model and tokenizer and category_mapping:
#             with st.spinner("Analyzing text sequences and attention weights..."):
#                 # --- Step A: Preprocess the input ---
#                 cleaned_text = clean_text(user_input)
#                 input_words = (
#                     cleaned_text.split()
#                 )  # Track raw tokens for plotting

#                 # If text is too short or empty after cleaning
#                 if len(input_words) == 0:
#                     st.warning("Please enter valid text content.")
#                 else:
#                     # Convert to sequence integers
#                     sequence = tokenizer.texts_to_sequences([cleaned_text])

#                     # Pull max sequence ceiling dynamically from tokenizer settings or default
#                     # Match this with the MAX_LEN used in your training phase (e.g., 150)
...
#                     predictions, weights_matrix = model.predict(
#                         padded_sequence, verbose=0
#                     )

#                     # Determine highest probability class
#                     predicted_class_idx = np.argmax(predictions[0])
#                     predicted_category = reverse_mapping[predicted_class_idx]
#                     confidence = predictions[0][predicted_class_idx] * 100

#                     # Display Prediction Metric Card
#                     st.metric(
...
#                     st.write("---")
#                     st.subheader("🧠 Word-to-Word Attention Heatmap")
#                     st.caption(
#                         "This matrix displays the internal relationships discovered by the Self-Attention mechanism."
#                     )

#                     # --- Step C: Extract & Plot Attention Scores ---
#                     # Average over head dimension mappings
#                     avg_weights = np.mean(weights_matrix[0], axis=0)

#                     # Limit matrix slice display to the actual number of words written (up to 15 max for clean viewing)
#                     display_limit = min(len(input_words), 15)
#                     heatmap_data = avg_weights[:display_limit, :display_limit]
#                     display_labels = input_words[:display_limit]

#                     # Standardize normalized score bounds for readable visual scaling color bounds
#                     if np.sum(heatmap_data) > 0:
#                         heatmap_data = heatmap_data / np.sum(
#                             heatmap_data, axis=1, keepdims=True
#                         )

#                     # Generate Seaborn Plot Figure
...
#                     # Render figure to Streamlit canvas container seamlessly
#                     st.pyplot(fig)
#         else:
#             st.error("Application configuration missing tracking components.")
#     elif submit_button:
#         st.warning("Please enter an article text snippet before processing.")
